module Parser (Parser, apply, sat, char, string, some, many, orelse, oneof, blank, keyword, token, token2, integer, float, allof) where
...
token2 :: Parser String
token2 = blank >> (some $ sat isAlphaNum)
